...
//			}
			...
			if( ig.input.pressed('grip') )//instead of state use pressed for single events
			{
				...
				//EntityBall ist provisorisch
				var newArm=ig.game.spawnEntity('EntityBall', this.pos.x+20+Math.cos(angle)*20, this.pos.y+20+Math.sin(angle)*20, null );
				newArm.vel.x=Math.cos(angle)*100;
				newArm.vel.y=Math.sin(angle)*100;
